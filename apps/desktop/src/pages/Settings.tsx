...
export function Settings(props: SettingsProps) {
  ...
  const trayTargetOptions: Array<{
    value: ProviderId | "max" | "last-viewed";
    label: string;
  }> = [
    ...
    ...settings.providerOrder
      ...
      .map((id) => {
        const state = providerStates.find(
          (providerState) => providerState.provider.id === id,
        );
        return {
          value: id,
        ...
        {tab === "display" ? (
          <div className="flex flex-col gap-1">
            ...
            <Row
              ...
              description={t("settings.display.compactModeDescription")}
            >
              <Switch
                checked={settings.compactMode}
                onChange={onCompactModeChange}
                ariaLabel={t("settings.display.compactModeToggle")}
              />
            </Row>
            <Row
              label={t("settings.display.alwaysVisible")}
              description={t("settings.display.alwaysVisibleDescription")}
            >
              <Switch
                checked={settings.alwaysVisible}
                onChange={onAlwaysVisibleChange}
                ariaLabel={t("settings.display.alwaysVisibleToggle")}
              />
            </Row>
          </div>
        ) : null}
        ...
        {tab === "providers" ? (
          <div className="flex flex-col gap-1">
            {providerStates.map((state) => {
              const enabled = !settings.disabledProviders.includes(
                state.provider.id,
              );
              return (
            ...
            {settings.featureFlags.localApiEnabled ? (
              <div className="flex flex-col gap-1 border-b border-border py-2 last:border-b-0">
                <span className="text-xs text-fg">
                  {t("settings.system.widgetRelayUrl")}
                </span>
                <input
                  value={settings.widgetSyncRelayUrl}
                  onChange={(event) =>
                    onWidgetSyncRelayUrlChange(event.target.value)
                  }
                ...
                />
                <span className="text-xs text-fg">
                  {t("settings.system.widgetSyncUrl")}
                </span>
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="flex flex-col gap-1 border-b border-border py-2 last:border-b-0">
              <div className="flex flex-col gap-0.5">
                <span className="text-xs text-fg">
                  {t("settings.notifications.trayTarget")}
                </span>
