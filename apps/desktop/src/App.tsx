...
export default function App() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setProviders(defs);
        await shell.setWindowPresentation(settings.alwaysVisible);
        await refresh(
          shell,
          setSnapshots,
          setRefreshing,
  ...
  const refreshAuto = useCallback(
    async (refreshIntervalMinutes: number) => {
      if (!shell) return;
      await refresh(
        shell,
        setSnapshots,
  ...
  const persistAndRefresh = useCallback(
    async (mutate: () => Promise<AppSettings>) => {
      if (!shell) return;
  ...
  useEffect(() => {
    ...
    if (!seededThresholds.current) {
      for (const state of snapshots) {
        ...
        thresholdMemory.current[state.provider.id] =
          preferences.notificationThresholds.filter(
    ...
    if (preferences.notificationsEnabled) {
      for (const state of snapshots) {
        ...
        thresholdMemory.current[state.provider.id] = updated.sort(
    ...
    void (async () => {
      ...
      if (
        ...
        settings = await shell.updatePreferences({
          widgetSyncPairId:
            preferences.widgetSyncPairId || createWidgetSyncPairId(),
  ...
  const handlers = {
    onToggleProvider: (id: ProviderId) =>
      shell
        ? void persistAndRefresh(() => shell.toggleProvider(id))
        : undefined,
    onThemeChange: (themeMode: ThemeMode) =>
      shell
        ? void persistOnly(() => shell.updatePreferences({ themeMode }))
        : undefined,
    onLocaleChange: (locale: Locale) =>
      shell
        ? void persistOnly(() => shell.updatePreferences({ locale }))
        : undefined,
    onDisplayModeChange: (displayMode: DisplayMode) =>
      shell
        ? void persistOnly(() => shell.updatePreferences({ displayMode }))
        : undefined,
    onCompactModeChange: (compactMode: boolean) =>
      shell
        ? void persistOnly(() => shell.updatePreferences({ compactMode }))
        : undefined,
    onAlwaysVisibleChange: (alwaysVisible: boolean) =>
      shell
        ? void persistOnly(() => shell.updatePreferences({ alwaysVisible }))
        : undefined,
    onRefreshIntervalChange: (
      ...
      shell
        ? void persistAndRefresh(() =>
            shell.updatePreferences({ refreshIntervalMinutes }),
          )
        : undefined,
    onNotificationsEnabledChange: (notificationsEnabled: boolean) =>
      shell
        ? void persistOnly(() =>
            shell.updatePreferences({ notificationsEnabled }),
          )
        : undefined,
    onNotificationThresholdsChange: (notificationThresholds: number[]) =>
      shell
        ? void persistOnly(() =>
            shell.updatePreferences({ notificationThresholds }),
          )
        : undefined,
    onTrayTargetChange: (trayTarget: ProviderId | "max" | "last-viewed") =>
      shell
        ? void persistOnly(() => shell.updatePreferences({ trayTarget }))
        : undefined,
    onStartOnLoginChange: (startOnLogin: boolean) =>
      shell
        ? void persistOnly(() => shell.updatePreferences({ startOnLogin }))
        : undefined,
    ...
    onWidgetSyncRelayUrlChange: (widgetSyncRelayUrl: string) =>
      shell
        ? void persistOnly(() =>
            shell.updatePreferences({ widgetSyncRelayUrl }),
          )
        : undefined,
  };
    ...
    void (async () => {
      try {
        ...
        await getCurrentWindow().setSize(
          new LogicalSize(
            home ? 1000 : 520,
            home ? (preferences.compactMode ? 92 : 112) : 620,
          ),
        );
        await shell?.setWindowPresentation(preferences.alwaysVisible);
      } catch {
  ...
  }, [activeView, preferences.alwaysVisible, preferences.compactMode, shell]);

  if (activeView === "home") {
    return (
      <IslandOverview
        states={snapshots.filter(
          (state) => !preferences.disabledProviders.includes(state.provider.id)
        )}
        settings={preferences}
        refreshing={refreshing}
        onRefresh={refreshNow}
        onAlwaysVisibleChange={handlers.onAlwaysVisibleChange!}
        t={t}
      />
    );
  }
  ...
  return (
    <div className="relative mx-auto flex h-screen max-h-screen w-full max-w-[520px] flex-col overflow-hidden rounded-2xl bg-bg shadow-panel">
          ...
          {activeView === "settings" ? (
            <Settings
              ...
              onNotificationThresholdsChange={
                handlers.onNotificationThresholdsChange!
              }
              onNotificationsEnabledChange={
                handlers.onNotificationsEnabledChange!
              }
